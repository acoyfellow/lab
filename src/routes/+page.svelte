...
<svelte:head>
  <title>Lab — Open source edge compute for agents</title>
  <meta name="description" content="Open source tool to run JavaScript in Cloudflare isolates with least-privilege capabilities. Self-host on your Cloudflare account. Every execution produces a shareable trace." />
</svelte:head>
...
<div class="max-w-3xl mx-auto px-6 py-10 max-sm:px-4 max-sm:py-8 space-y-12">
  ...
  <section class="space-y-5" aria-labelledby="hero">
    ...
    <h1 id="hero" class="text-[1.65rem] sm:text-[2.25rem] font-semibold tracking-tight leading-[1.15]">
      Run untrusted code at the edge.<br />
      <span class="text-(--accent)">Prove what happened.</span>
    </h1>
  ...
  <section class="flex flex-wrap gap-4 pt-4 border-t border-(--border)" aria-labelledby="links">
    ...
    <a href="https://github.com/acoyfellow/lab" target="_blank" rel="noopener noreferrer" class="text-[0.8125rem] text-(--text-2) hover:text-(--text)">GitHub</a>
  </section>
</div>
