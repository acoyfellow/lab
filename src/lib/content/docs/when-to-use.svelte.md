...
- **Receipt-first** debugging: you want a persisted document per run with inputs/outputs for inspection and sharing.
...
Lab **orchestrates** Loader isolates, capabilities, and receipts. It does not replace Workers; it **depends** on them. See [Architecture](/docs/architecture) for loaders, KV snapshots, spawn, and `/invoke/*`.
